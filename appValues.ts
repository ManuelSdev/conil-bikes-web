...
type SpaState = 'pendiente' | 'activa' | 'finalizada' | 'cancelada'
...
export const bookingState: Record<BookingStates, SpaState> = {
   [BookingStates.PENDING]: 'pendiente',
   [BookingStates.ACTIVE]: 'activa',
   [BookingStates.FINISHED]: 'finalizada',
   [BookingStates.CANCELLED]: 'cancelada',
}
...
const typeMap = new Map<BikeType, string>([
   ['mountain', 'montaña'],
   ['city', 'paseo'],
   ['electric', 'eléctrica'],
   ['road', 'carretera'],
...
const rangeMap = new Map<BikeRange, string>([
   ['top', 'premium'],
   ['high', 'alta'],
   ['mid', 'media'],
   ['ride', 'paseo/trekking'],
